...
def sort_file(source_path:str):
    for root_dir,sub_dir,filenames in os.walk(source_path):
        for filenames in filenames:
            file_path:str = os.path.join(root_dir, filenames)
            extension:str = os.path.splitext(filenames)[1]

            if extension:
                target_folder:str = create_folder(source_path,extension)
                target_path: str = os.path.join(target_folder,filenames)

                shutil.move(file_path, target_path)

def remove_empty_folders(source_path:str):
    #removes all the empty file in the folder if it exists
